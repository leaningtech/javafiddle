...
<aside class="w-10 bg-stone-100 dark:bg-stone-800 flex-shrink-0 transition-[width] flex flex-col overflow-hidden" class:!w-80={$isSidebarOpen}>
	<div class="text-right shadow-none animate-shadow" class:!shadow={$isSidebarOpen}>
		<button class="w-10 h-10 text-stone-600 hover:text-stone-900 dark:text-stone-400 dark:hover:text-stone-100 inline-flex items-center justify-center" on:click={() => $isSidebarOpen = !$isSidebarOpen}>
			{#if $isSidebarOpen}
	...
	<div class="w-80 grow overflow-hidden">
		...
		{#if $isSidebarOpen}
			<div class="h-1/2 overflow-y-auto flex flex-col" transition:blur={{ duration: 150 }}>
				<div class="grow p-4 leading-tight bg-stone-200 text-stone-700 dark:bg-stone-700 dark:text-stone-300 text-sm">
					<p>
					...
					<div class="mt-6 flex items-center justify-center gap-2">
						...
						<a class="rounded hover:bg-stone-800 text-stone-950 dark:text-white hover:text-white font-semibold px-3 py-2 inline-flex items-center justify-center gap-2" href="https://github.com/leaningtech/javafiddle">
							<Icon icon="fa-brands:github" class="w-4 h-4" />
							View source
						</a>
					</div>
				</div>
				<a class="p-6 border-t-4 border-orange-500 bg-stone-300 dark:bg-stone-600 text-stone-800 dark:text-stone-100" href="https://labs.leaningtech.com/cheerpj">
